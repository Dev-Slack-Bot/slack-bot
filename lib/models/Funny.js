...
module.exports = class Funny {
  ...
  static async updateViewsById(id, { timesViewed }) {
    const { rows } = await pool.query('UPDATE funny SET times_viewed = $2 WHERE funny.id = $1  RETURNING entree, course_id, times_viewed', [id, timesViewed]);
    return new Funny(rows[0]);
  }
};
