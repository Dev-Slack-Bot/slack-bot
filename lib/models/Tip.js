...
module.exports = class TipClass {
    ...
    static async mostViewedTips() {
      const { rows } = await pool.query('SELECT tip, tip_url, course.course, times_viewed FROM tips LEFT JOIN course ON tips.course_id = course.id ORDER BY times_viewed LIMT 10');
